...
func capitalise(_ value: Any?) -> Any? {
    if let array = value as? [Any?] {
        array.map { stringify($0).capitalized }
    } else {
        stringify(value).capitalized
    }
}

func uppercase(_ value: Any?) -> Any? {
    if let array = value as? [Any?] {
        array.map { stringify($0).uppercased() }
    } else {
...
func lowercase(_ value: Any?) -> Any? {
    if let array = value as? [Any?] {
        array.map { stringify($0).lowercased() }
    } else {
        stringify(value).lowercased()
    }
}
...
func defaultFilter(value: Any?, arguments: [Any?]) -> Any? {
    ...
    if let value, String(describing: value) != "nil" {
        return value
    }
