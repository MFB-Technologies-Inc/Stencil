...
public class TokenParser {
    ...
    private(set) var parsedTokens: [Token] = []
    fileprivate let environment: Environment
    private var previousWhiteSpace: WhitespaceBehaviour.Behaviour?
    ...
    public func parse(_ parseUntil: ((_ parser: TokenParser, _ token: Token) -> (Bool))?) throws -> [NodeType] {
        ...
        while !tokens.isEmpty {
            ...
            case .block:
                ...
                if let parseUntil, parseUntil(self, token) {
                    prependToken(token)
                    return nodes
                }
...
extension Environment {
    ...
    func findFilter(_ name: String) throws -> FilterType {
        ...
        let suggestedFilters = suggestedFilters(for: name)
    ...
    private func suggestedFilters(for name: String) -> [String] {
        let allFilters = extensions.flatMap(\.filters.keys)
        ...
        return filtersWithDistance.filter { $0.distance == minDistance }.map(\.filterName)
    }
