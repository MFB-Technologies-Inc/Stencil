...
public struct Variable: Equatable, Resolvable {
    ...
    public func resolve(_ context: Context) throws -> Any? {
           ...
           (variable.hasPrefix("\"") && variable.hasSuffix("\""))
        {
            ...
            return String(variable[
                variable.index(after: variable.startIndex) ..< variable
                    .index(before: variable.endIndex)
            ])
        }
    ...
    private func resolve(bit: String, collection: some Collection) -> Any? {
        if let index = Int(bit) {
            ...
            collection[collection.index(collection.endIndex, offsetBy: -1)]
        } else if bit == "count" {
            collection.count
        } else {
            nil
...
public struct RangeVariable: Resolvable {
    ...
    public init?(_ token: String, environment: Environment) throws {
        ...
        from = try environment.compileFilter(components[0])
        to = try environment.compileFilter(components[1])
    }
    ...
    public init?(_ token: String, environment: Environment, containedIn containingToken: Token) throws {
        ...
        from = try environment.compileFilter(components[0], containedIn: containingToken)
        to = try environment.compileFilter(components[1], containedIn: containingToken)
    }
